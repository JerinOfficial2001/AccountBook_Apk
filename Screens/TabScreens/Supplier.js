...
export default function Supplier({navigation}) {
  ...
  return (
    <PageLayout>
      <FlatList
        ...
        renderItem={data => {
          return (
            <UserCard
              onClick={() =>
                navigation.navigate('Collections', {type: 'SUPPLIER'})
              }
            />
          );
        }}
      ...
      />
      <Button
        onPress={() => navigation.navigate('AddParty', {type: 'SUPPLIER'})}
        ...
        <Text style={{fontWeight: 'bold', fontSize: 16}}>ADD SUPPLIER</Text>
      </Button>
    </PageLayout>
  );
}
